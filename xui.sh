...
install() {
    bash <(curl -Ls https://raw.githubusercontent.com/gurupandit420/Wg/master/xui.sh)
    if [[ $? == 0 ]]; then
        if [[ $# == 0 ]]; then
            start
        else
...
install_bbr() {
    ...
    bash <(curl -L -s https://raw.githubusercontent.com/gurupandit420/Wg/master/bbr.sh)
    echo ""
    before_show_menu
}

update_shell() {
    wget -O /usr/bin/x-ui -N --no-check-certificate https://github.com/gurupandit420/Wg/master/xui.sh
    if [[ $? != 0 ]]; then
        echo ""
